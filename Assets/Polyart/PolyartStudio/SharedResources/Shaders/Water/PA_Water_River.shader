	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float4 vDisplacement160 = ( _HeightIntensity * pow(abs(( tex2Dlod( _HeightMap, float4( panner9_g69, 0, 0.0) ) +
              tex2Dlod( _HeightMap, float4( panner13_g69, 0, 0.0) ) +
              tex2Dlod( _HeightMap, float4( panner18_g69, 0, 0.0) )
            ) * 0.25),temp_cast_0) ) * float4( v.ase_normal , 0.0 );
						 ...
						 ) : SV_Target
			{
				...
				float2 panner18_g69 = ( temp_output_7_0_g69 * float2( -0.1,0.1 ) + ( temp_output_2_0_g69 + float2( 0.85,0.14 ) ));
				float4 temp_cast_0 = (_HeightPower).xxxx;
				float4 vDisplacement160 = (
    _HeightIntensity * pow(
        abs(
            ( tex2D( _HeightMap, panner9_g69 ) +
              tex2D( _HeightMap, panner13_g69 ) +
              tex2D( _HeightMap, panner18_g69 )
            ) * 0.25
        ),
        temp_cast_0
    )
) * float4( IN.ase_normal, 0.0 );
